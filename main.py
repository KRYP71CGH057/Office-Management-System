...
UPLOAD_FOLDER = 'C:\\Users\\HP\\Downloads\\LORD EL\\Office-Management-System\\files'
...
def get_db_connection():
    """Create a database connection that works for both development and production"""
    # For Heroku/Render, they use postgres:// but we need postgresql:// for compatibility
    db_url = os.getenv('DATABASE_URL', 'database.db')
    
    # For SQLite (development)
    if db_url == 'database.db' or db_url.startswith('sqlite:///'):
        return sqlite3.connect('database.db')
    # For PostgreSQL (production on Heroku/Render)
    else:
        # We'll continue using SQLite for now since the app is built for it
        # In a real production environment, you'd need to switch to psycopg2
        return sqlite3.connect('database.db')
# ==============================================
